...
// - Upload logo/favicon -> S3 (MinIO) + guarda URL en DB
...
async function updateBranding(req, res, next) {
  try {
    ...
    await sequelize.query(
      `UPDATE shop_branding SET name=?, updated_at=NOW() WHERE id=1`,
      { replacements: [name] }
    );
...
async function uploadLogo(req, res, next) {
  try {
    ...
    const file = req.file;
    if (!file) return res.status(400).json({ ok: false, code: "FILE_REQUIRED", message: "Falta el archivo (file)." });

    const up = await uploadShopAsset({ file, kind: "logo" });

    await sequelize.query(
      `UPDATE shop_branding SET logo_url=?, updated_at=NOW() WHERE id=1`,
      { replacements: [up.url] }
    );
...
async function uploadFavicon(req, res, next) {
  try {
    ...
    await sequelize.query(
      `UPDATE shop_branding SET favicon_url=?, updated_at=NOW() WHERE id=1`,
      { replacements: [up.url] }
    );
