  ...
  try {
    const result = await sequelize.transaction(async (t) => {
      ...
      return {
        ok: true,
        request_id,
        order: { id: order_id, public_code, status: "created", fulfillment_type, branch_id, subtotal, shipping_total, total, payment_status: orderPayStatus },
        payment: { id: pay.payment_id, provider: pay.provider, status: isMp ? "pending" : pay.status, external_reference: externalRef },
        redirect_url,
        mp,
      };
    });
    ...
    log(request_id, "OK order=", { id: result?.order?.id, public_code: result?.order?.public_code, redirect: !!result?.redirect_url });
    ...
    if (e?.code === "MISSING_STOCK_VIEW") {
      ...
      return res.status(500).json({ message: "No se puede validar stock por sucursal.", code: "MISSING_STOCK_VIEW", detail, request_id });
    }
