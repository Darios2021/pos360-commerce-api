...
//
// Admin payments ecommerce (gestión)
// - Lista + filtros
...
async function listAdminShopPayments(req, res) {
  ...
  try {
    ...
    if (method) {
      cond.push(`LOWER(p.method) = :method`);
      repl.method = method;
    }
    if (public_code) {
    ...
    const [rows] = await sequelize.query(
      `
      SELECT
        p.id, p.order_id, p.provider, p.method, p.status, p.amount, p.currency,
        p.reference, p.note,
        p.external_reference, p.external_id,
        p.mp_preference_id, p.mp_payment_id, p.mp_merchant_order_id,
        p.external_status, p.status_detail, p.payer_email,
        p.proof_url, p.bank_reference,
        p.reviewed_by, p.reviewed_at, p.review_note,
    ...
    return res.json({ ok: true, meta: { total, pages, page, limit }, items: rows || [] });
  } catch (e) {
  ...
  try {
    const result = await sequelize.transaction(async (t) => {
      ...
      const map = [
        ...
        ["external_reference", "external_reference", (v) => toStr(v) || null],
        ["mp_payment_id", "mp_payment_id", (v) => toStr(v) || null],
        ["mp_merchant_order_id", "mp_merchant_order_id", (v) => toStr(v) || null],
      ];
      ...
      // si tocaron algo => reviewed_by/at
      if (sets.length) {
