...
exports.list = async (req, res) => {
  ...
  const { rows, count } = await Product.findAndCountAll({
    where,
    include: [
      { model: Category, as: "category", attributes: ["id", "name", "parent_id"] },
...
exports.create = async (req, res) => {
  ...
  const item = await Product.create({
    ...
    cost: body.cost ?? 0,
    price: body.price ?? (body.list_price ?? 0), // compat
...
exports.update = async (req, res) => {
  ...
  await item.update({
    ...
    list_price: body.list_price ?? item.list_price,
    cash_price: body.cash_price ?? item.cash_price,
    reseller_price: body.reseller_price ?? item.reseller_price,
    promo_price: body.promo_price ?? item.promo_price,
