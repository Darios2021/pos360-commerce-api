...
const categoryInclude = () => ({
  model: Category,
  // ✅ tu asociación real en models/index.js es "category"
  as: "category",
...
exports.list = async (req, res, next) => {
  try {
    ...
    // ✅ si en algún lado quedó "subcategory", evitamos que te tumbe prod:
    if (String(e?.name || "").includes("SequelizeEagerLoadingError")) {
      return res.status(500).json({
        ok: false,
        code: "EAGER_LOADING_ALIAS_ERROR",
        message:
          "Revisá include/as en Product->Category. El alias correcto es 'category'.",
        details: e.message,
      });
    }
    next(e);
