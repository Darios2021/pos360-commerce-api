...
// ✅ COPY-PASTE FINAL
// Public Controller – Ecommerce (Catalog + Suggestions + Taxonomy)
...
module.exports = {
  ...
  async listSuggestions(req, res) {
    try {
      ...
      // comportamiento tipo Mercado Libre
      if (q.length < 2) {
        return res.json({ ok: true, items: [] });
      }

      const items = await PublicService.listSuggestions({
        branch_id,
        q,
        limit,
      });
  ...
  async getProductById(req, res) {
    try {
      ...
      const item = await PublicService.getProductById({
        branch_id,
        product_id,
      });
